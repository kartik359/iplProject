  ...
  return (
    <>
      <div className="points-table-container">
        ...
        {staticData?.pointsTable[0]?.pointsTableInfo?.map((data, i) => (
          <div className="table-row" key={i}>
            <span>{i + 1}</span>
            <span className="teamName">{<img src={`https://res.cloudinary.com/digkgdovw/image/upload/v1715267905/iplTeamLogo/${data.teamImageId}`} alt="Team logo" />}  {data.teamName}  {(data.teamQualifyStatus === "Q") ? "(Q)" : ""}  </span>


            <span>{data.matchesPlayed}</span>
            <span>{data.matchesWon}</span>
            <span>{data.matchesLost}</span>
            <span className="points">{data.points}</span>
            <span className="nrr">{data.nrr}</span>
            <span className="form">
              {data.form.reverse().map((f, idx) => (
                <span key={idx} className={f === "W" ? "win" : "loss"}>{f}</span>
              ))}
            </span>
          </div>
        ))}
